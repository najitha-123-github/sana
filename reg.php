...
<body>
    ...
    <div class="form-one form-step active">
        ...
        <div>
            ...
            <textarea name="address" rows="7" cols="50" placeholder="e.g. house name, p.o, city" required></textarea>
        </div>
        <div>
            <label>Password</label><br>
            <input name="password" rows="7" cols="50" placeholder="Enter Password" required></input>
        </div>
        <div>
            <label>Mobile</label>
...
if (isset($_POST['register'])) {
    ...
    if (empty($name) || strlen($name) < 3) {
        echo "<script>alert('Please enter a valid name with at least 3 characters.');</script>";
        $error = true;
    }
    ...
    if (empty($address)) {
        echo "<script>alert('Please provide a valid address.');</script>";
        $error = true;
    }

    if (empty($complaint_type)) {
        echo "<script>alert('Please select a valid complaint type.');</script>";
        $error = true;
    }

    if (empty($complaint_detail)) {
        echo "<script>alert('Please provide details of the complaint.');</script>";
        $error = true;
    }

    // Removed photo validation entirely
    $targetFile = '';  // No validation, the file is optional

    if (!$error) {
        // Insert complaint details into database
        $sql_complaints = "INSERT INTO `complaints`(`name`, `dob`, `address`, `phno`, `complaint_type`, `date`, `complaint_detail`, `photo`, `suspect_name`, `details`, `email`, `status`, `officerid`) 
                            VALUES ('$name','$dob','$address','$mobile','$complaint_type','$date','$complaint_detail','$targetFile','$suspect_name','$suspect_detail','$email','pending',0)";
        
        $sql_users = "INSERT INTO `users`(`name`, `email`, `dob`, `address`, `phno`, `password`) 
                      VALUES ('$name','$email','$dob','$address','$mobile', '$password')";
        
        $sql_login = "INSERT INTO `login`(`email`, `password`, `usertype`) 
